...
export default function BookingsToday() {
  ...
  const loadBookings = () => {
    ...
    const data = getBookings({ date: today, search: search || undefined, status: statusFilter || undefined, serviceType: serviceFilter || undefined });
    setBookings(data);
    setLoading(false);
  };
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-0">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900 tracking-tight">{t('today.title')}</h1>
      ...
      <div className="bg-white p-3 rounded-2xl md:rounded-[1.5rem] border border-slate-200/60 shadow-[0_4px_20px_rgb(0,0,0,0.02)] flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            placeholder={t('today.search.ph') as string}
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative w-full sm:w-auto">
            ...
            >
              <option value="">{t('today.filter.status')}</option>
          ...
          <div className="relative w-full sm:w-auto">
            ...
            >
              <option value="">{t('today.filter.service')}</option>
      ...
      <div className="bg-white/50 md:bg-white border-0 md:border md:border-slate-200 md:rounded-[2rem] overflow-hidden md:shadow-[0_4px_20px_rgb(0,0,0,0.02)]">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-64 space-y-4 bg-white rounded-3xl md:rounded-none">
            ...
            <p className="text-slate-500 font-medium">{t('today.loading')}</p>
          </div>
        ) : bookings.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-3xl md:rounded-none">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-2">{t('today.empty.title')}</h3>
            <p className="text-slate-500 max-w-sm mb-8">{t('today.empty.desc')}</p>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-left text-sm whitespace-nowrap">
                <thead className="bg-slate-50 border-b border-slate-100 text-slate-500">
                  <tr>
                    <th className="px-6 py-5 font-bold tracking-wide">{t('today.th.time')}</th>
                    <th className="px-6 py-5 font-bold tracking-wide">{t('today.th.customer')}</th>
                    <th className="px-6 py-5 font-bold tracking-wide">{t('today.th.vehicle')}</th>
                    <th className="px-6 py-5 font-bold tracking-wide">{t('today.th.status')}</th>
                    <th className="px-6 py-5 font-bold tracking-wide text-right">{t('today.th.action')}</th>
                  </tr>
                </thead>
                  ...
                  {bookings.map((b, index) => (
                    ...
                    >
                      <td className="px-6 py-5">
                        <div className="inline-flex items-center justify-center bg-slate-100 text-slate-900 font-bold px-3 py-1.5 rounded-lg">{b.bookingTime}</div>
                      </td>
                      <td className="px-6 py-5">
                      ...
                      <td className="px-6 py-5 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={(e) => { e.preventDefault(); handleNotify(b.customerPhone, b.customerName, b.carType); }} className="p-2 text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition-colors" title="WhatsApp">
                            <MessageCircle className="w-5 h-5" />
                          </button>
                          <Link to={`/admin/bookings/${b.id}`}>
                            <button className="p-2 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors" title={t('today.card.detail') as string}>
              ...
              {bookings.map((b, index) => (
                ...
                >
                  ...
                  <div className="bg-slate-50 p-3 rounded-2xl mb-4 border border-slate-100">
                    ...
                    <div className="text-xs text-slate-500 uppercase tracking-wider mt-0.5">{b.plateNumber || t('today.noplate')}</div>
                  </div>
                  <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
                    <button onClick={(e) => { e.preventDefault(); handleNotify(b.customerPhone, b.customerName, b.carType); }} className="flex-1 flex justify-center items-center gap-2 py-3 bg-emerald-50 text-emerald-600 rounded-xl font-semibold text-sm hover:bg-emerald-100 transition-colors">
                      <MessageCircle className="w-4 h-4" /> {t('today.card.contact')}
                    </button>
                    <Link to={`/admin/bookings/${b.id}`} className="flex-1">
                      <button className="w-full flex justify-center items-center gap-2 py-3 bg-slate-100 text-slate-700 rounded-xl font-semibold text-sm hover:bg-slate-200 transition-colors">
                        {t('today.card.detail')} <ChevronRight className="w-4 h-4" />
                      </button>
                    </Link>
                  </div>
                </motion.div>
